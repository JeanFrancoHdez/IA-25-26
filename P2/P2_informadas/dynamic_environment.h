...
struct DynamicResult {
  ...
  DynamicResult() : success(false), total_cost(0.0), total_steps(0), 
     total_replans(0), failed_attempts(0) {}
};
...
public:
  ...
  std::string GenerateCompleteReport(const DynamicResult& result, 
         const Position& start, const Position& goal) const;
  
  // Guardar resultados en archivo
  bool SaveResultsToFile(const DynamicResult& result, const std::string& filename,
        const Position& start, const Position& goal) const;
...
private:
  ...
  bool ExecuteStep(Position& current_pos, const std::vector<Position>& planned_path, 
    size_t& step_index, DynamicResult& result);
};
