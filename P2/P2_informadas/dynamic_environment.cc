...
std::string DynamicEnvironment::GenerateCompleteReport(const DynamicResult& result, 
  ...
  oss << "  Proporción media de obstáculos: " << std::fixed << std::setprecision(3) 
  << avg_obstacle_ratio * 100 << "%\n\n";
  
  // Detalle de cada búsqueda A*
  ...
  oss << "    Ratio obstáculos: " << std::fixed << std::setprecision(1) 
  << result.obstacle_ratios[i] * 100 << "%\n";
  }
  oss << "\n";
  }
...
bool DynamicEnvironment::SaveResultsToFile(const DynamicResult& result, const std::string& filename,
              const Position& start, const Position& goal) const {
