...
// MODIFICACIÓN
AStarResult AStar::Search3Best(const Position& start, const Position& goal, bool verbose) {
  Reset();
  ...
  while (!open_list_.empty()) {
    ...
    // Extraer los 3 mejores nodos (menores f)
    std::vector<std::shared_ptr<AStarNode>> top3;
    ...
    for (int i = 0; i < 3 && !temp_queue.empty(); ++i) {
      top3.push_back(temp_queue.top());
      temp_queue.pop();
    }
    
    // Seleccionar uno al azar entre los 3 mejores
    std::uniform_int_distribution<int> dist(0, top3.size() - 1);
    int random_index = dist(rng);
    auto current = top3[random_index];
