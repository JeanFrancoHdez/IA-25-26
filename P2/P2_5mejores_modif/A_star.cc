...
AStarResult AStar::Search5Best(const Position& start, const Position& goal, bool verbose) {
  Reset();
  ...
  while (!open_list_.empty()) {
    ...
    // Extraer los 5 mejores nodos (menores f)
    std::vector<std::shared_ptr<AStarNode>> top5;
    std::priority_queue<std::shared_ptr<AStarNode>, std::vector<std::shared_ptr<AStarNode>>, AStarNodeComparator> temp_queue = open_list_;
    
    for (int i = 0; i < 5 && !temp_queue.empty(); ++i) {
      top5.push_back(temp_queue.top());
      temp_queue.pop();
    }
    
    // Seleccionar uno al azar entre los 5 mejores
    std::uniform_int_distribution<int> dist(0, top5.size() - 1);
    int random_index = dist(rng);
    auto current = top5[random_index];
    
    // Remover el nodo seleccionado de open_list
    std::priority_queue<std::shared_ptr<AStarNode>, std::vector<std::shared_ptr<AStarNode>>, AStarNodeComparator> new_open_list;
    temp_queue = open_list_;
    
    Position current_pos = current->GetPosition();
    while (!temp_queue.empty()) {
      auto node = temp_queue.top();
      temp_queue.pop();
      if (!(node->GetPosition() == current_pos)) {
