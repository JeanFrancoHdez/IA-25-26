...
SearchResult BFS::Search(int start, int goal) {
  ...
  while (!frontier_.empty()) {
    std::shared_ptr<Node> current_node = SelectBestNode();
...
std::shared_ptr<Node> BFS::SelectBestNode() {
  ...
  // Buscar los dos mejores en toda la frontera
  for (size_t i = 2; i < frontier_.size(); ++i) {
    double current_cost = frontier_[i]->GetPathCost();
    
    if (current_cost < frontier_[best_idx]->GetPathCost()) {
      // Nuevo mejor nodo encontrado
      second_best_idx = best_idx;
      best_idx = i;
    } else if (current_cost < frontier_[second_best_idx]->GetPathCost()) {
      // Nuevo segundo mejor nodo encontrado
      second_best_idx = i;
    }
  }
  
  // Seleccionar aleatoriamente entre los dos mejores
  static std::random_device rd;
  static std::mt19937 gen(rd());
  std::uniform_int_distribution<> dis(0, 1);
  
  int selected_idx = (dis(gen) == 0) ? best_idx : second_best_idx;
