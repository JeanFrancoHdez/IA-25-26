...
// Constructor
SearchAlgorithm::SearchAlgorithm(const Graph* graph) 
  : graph_(graph), current_iteration_(0) {
}
...
// Marca un nodo como generado
void SearchAlgorithm::MarkNodeGenerated(int vertex) {
  if (std::find(generated_nodes_.begin(), generated_nodes_.end(), vertex) == generated_nodes_.end()) {
    generated_nodes_.push_back(vertex);
  }
}

// Marca un nodo como inspeccionado
...
// Verifica si un nodo ya fue generado
bool SearchAlgorithm::IsNodeGenerated(int vertex) const {
  return std::find(generated_nodes_.begin(), generated_nodes_.end(), vertex) != generated_nodes_.end();
}

// Crea un nodo compartido
std::shared_ptr<Node> SearchAlgorithm::CreateNode(int vertex, std::shared_ptr<Node> parent, 
                                                  double path_cost, int depth) {
  return std::make_shared<Node>(std::to_string(vertex), parent, path_cost, depth);
}
...
std::vector<int> SearchAlgorithm::ReconstructPath(std::shared_ptr<Node> goal_node) const {
  ...
  while (current != nullptr) {
    path.insert(path.begin(), std::stoi(current->GetState()));
...
double SearchAlgorithm::CalculatePathCost(const std::vector<int>& path) const {
  ...
  for (size_t i = 0; i < path.size() - 1; ++i) {
    ...
    if (edge_cost < 0) {
      // No debería pasar si el camino es válido
      std::cerr << "Error: Arista inválida entre " << path[i] << " y " << path[i + 1] << std::endl;
      return -1.0;
    }
...
// Genera el reporte en formato string
std::string SearchAlgorithm::GenerateReport(const SearchResult& result, int start, int goal) const {
  ...
  oss << "Algoritmo: " << GetAlgorithmName() << "\n\n";
...
// Guarda el resultado en archivo
bool SearchAlgorithm::SaveResultToFile(const SearchResult& result, int start, int goal, 
  ...
  file << GenerateReport(result, start, goal);
  file.close();
